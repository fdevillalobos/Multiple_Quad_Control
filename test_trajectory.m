...
if vis
    h_fig = figure('Name', 'Environment', 'units','normalized','outerposition',[0 0 1 1]); hold on;
else
    h_fig = figure('Name', 'Environment', 'Visible', 'Off', 'units','normalized','outerposition',[0 0 1 1]); hold on;
end
for qn = 1:nquad
...
x = x0;        % state
copter_distance = zeros(3,3,1);
desired_state = cell(qn,1);
%% ************************* RUN SIMULATION *************************
...
for iter = 1:max_iter
    ...
    for qn = 1:nquad
        ...
        if iter == 1
            ...
            desired_state{qn} = trajhandle(time, qn);
            QP{qn}.UpdateQuadPlot(x{qn}, [desired_state{qn}.pos; desired_state{qn}.vel], time);
            h_title = title(sprintf('iteration: %d, time: %4.2f', iter, time));
        end
        
        %Calculation of distances between copters
        for d = 1:nquad
            copter_distance(qn,d) = sqrt(sum((x{qn}(1:3)-x{d}(1:3)).^2));
        end
        
        % Run simulation
        [tsave, xsave] = ode45(@(t,s) quadEOM(t, s, qn, controlhandle, trajhandle, params), timeint, x{qn});
        x{qn} = xsave(end, :)';
        %x{qn}(1:3) contains the actual position of the quad.
    
        % Update quad plot
        desired_state{qn} = trajhandle(time+cstep, qn);
        QP{qn}.UpdateQuadPlot(x{qn}, [desired_state{qn}.pos; desired_state{qn}.vel], time + cstep);
