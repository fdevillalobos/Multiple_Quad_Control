function [path, num_expanded, or_path] = dijkstra(map, start, goal, astar, draw)
%function [path, num_expanded, Val_Grid, Vis_Grid, or_path] = dijkstra(map, start, goal, astar, draw)
...
Vis_Grid = ones(size(map.grid) + 2*pad_value);
...
Val_Grid = ones(size(map.grid)+ 2*pad_value)*inf;
Val_Grid(Start(1),Start(2),Start(3)) = 0;

if(Vis_Grid(Goal(1),Goal(2),Goal(3)) == 1)
...
if(do_break == 0 && astar == false)
    while(Vis_Grid(Goal(1), Goal(2), Goal(3)) == 0)
        ...
        if(min(min(min(Vis_Inf))) == inf)
            do_break = 1; break;                                                %Path Blocked, Exit Routine.
        end
        
        [~, ind2] = min(Vis_Inf(:));                                        % Find Minimum coordinates of the minimim values in the path till now
        [x_coord, y_coord, z_coord] = ind2sub(size(Vis_Inf), ind2);         % Get the coordinates of the minimum values.
    end
    
elseif(do_break == 0 && astar == true)
    do_break = 0;
    while(Vis_Grid(Goal(1), Goal(2), Goal(3)) == 0)
        ActP      = [nrow, y_coord, z_coord];
        AuxM      = Val_Grid(ActP(1),ActP(2),ActP(3)) + Cmp_Mat;                                    %Matrix that contains distances + value of center.
        Insp_Box  = Val_Grid(ActP(1)-1:ActP(1)+1, ActP(2)-1:ActP(2)+1, ActP(3)-1:ActP(3)+1);        %Inpection Box centered in ActP.
        Not_Vis   = Vis_Grid(ActP(1)-1:ActP(1)+1, ActP(2)-1:ActP(2)+1, ActP(3)-1:ActP(3)+1) == 0;   %Which of those where not visited.
        GRT_val   = Insp_Box > AuxM;                                                                %Which pixels are greater.
        ...
        [nrow, ncol] = find(Vis_Inf ~= inf);                                %Find every index for values that we KNOW distance but havent visited yet.
        ...
        Value_Points = [nrow y_coord z_coord];
        Vis_InfDist = Vis_Inf;
        ...
        [~, ind2] = min(Vis_Inf(:));
        [nrow, y_coord, z_coord] = ind2sub(size(Vis_Inf), ind2);
    end
end
...
if(do_break == 0)
    ...
    while(Val_Grid(ActP(1), ActP(2), ActP(3)) ~= 0)                         %Trace Back the closer Points to Start.
        ...
        [nrow, y_coord, z_coord] = ind2sub(size(Insp_Box), ind2);           % Converts those indeces to x y z.
        ActP = [ActP(1)+nrow-2, ActP(2)+y_coord-2, ActP(3)+z_coord-2];      % Gets new point to trace back.
    ...
    if(draw > 0)
        ...
        %Vis_Grid2 = Vis_Grid(1+pad_value:pad_value+size(map.grid,1), 1+pad_value:pad_value+size(map.grid,2),1+pad_value:pad_value+size(map.grid,3));
